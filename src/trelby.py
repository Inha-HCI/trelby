...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        fileMenu = wx.Menu()
        fileMenu.Append(ID_FILE_NEW, u"새 문서\tCTRL-N")
        fileMenu.Append(ID_FILE_OPEN, u"열기\tCTRL-O")
        fileMenu.Append(ID_FILE_SAVE, u"저장하기\tCTRL-S")
        fileMenu.Append(ID_FILE_SAVE_AS, u"다른이름으로 저장하기")
        fileMenu.Append(ID_FILE_CLOSE, u"닫기\tCTRL-W")
        #fileMenu.Append(ID_FILE_REVERT, "Revert")
        fileMenu.AppendSeparator()
        fileMenu.Append(ID_FILE_IMPORT, u"불러오기")
        fileMenu.Append(ID_FILE_EXPORT, u"내보내기")
        fileMenu.AppendSeparator()
        fileMenu.Append(ID_FILE_PRINT, u"인쇄\tCTRL-P")
        fileMenu.AppendSeparator()

        tmp = wx.Menu()
        tmp.Append(ID_SETTINGS_CHANGE, u"변경")
        tmp.AppendSeparator()
        ...
        fileMenu.AppendSeparator()
        fileMenu.AppendSeparator()
        fileMenu.Append(ID_FILE_EXIT, u"프로그램 종료\tCTRL-Q")

        editMenu = wx.Menu()
        editMenu.Append(ID_EDIT_UNDO, u"이전\tCTRL-Z")
        editMenu.Append(ID_EDIT_REDO, u"다음\tCTRL-Y")
        editMenu.AppendSeparator()
        editMenu.Append(ID_EDIT_CUT, u"잘라내기\tCTRL-X")
        editMenu.Append(ID_EDIT_COPY, u"복사\tCTRL-C")
        editMenu.Append(ID_EDIT_PASTE, u"붙여넣기\tCTRL-V")
        editMenu.AppendSeparator()

        tmp = wx.Menu()
        tmp.Append(ID_EDIT_COPY_TO_CB, u"서식없음")
        tmp.Append(ID_EDIT_COPY_TO_CB_FMT, u"서식있음")

        editMenu.AppendMenu(ID_EDIT_COPY_SYSTEM, u"복사 (시스템)", tmp)
        editMenu.Append(ID_EDIT_PASTE_FROM_CB, u"붙여넣기 (시스템)")
        editMenu.AppendSeparator()
        editMenu.Append(ID_EDIT_SELECT_SCENE, u"&씬 선택")
        editMenu.Append(ID_EDIT_SELECT_ALL, u"전체 선택")
        editMenu.Append(ID_EDIT_GOTO_PAGE, u"&페이지로 이동tCTRL-G")
        editMenu.Append(ID_EDIT_GOTO_SCENE, u"씬으로 이동\tALT-G")
        #editMenu .AppendSeparator()
        #editMenu.Append(ID_EDIT_INSERT_NBSP, "Insert non-breaking space")
        editMenu.AppendSeparator()
        editMenu.Append(ID_EDIT_FIND, u"찾기 & 바꾸기\tCTRL-F")
        editMenu.AppendSeparator()
        editMenu.Append(ID_EDIT_DELETE_ELEMENTS, u"요소 삭제")

        viewMenu = wx.Menu()
        viewMenu.AppendRadioItem(ID_VIEW_STYLE_DRAFT, u"드래프트")
        viewMenu.AppendRadioItem(ID_VIEW_STYLE_LAYOUT, u"레이아웃")
        viewMenu.AppendRadioItem(ID_VIEW_STYLE_SIDE_BY_SIDE, u"나란히")
        ...
        viewMenu.AppendCheckItem(ID_VIEW_SHOW_FORMATTING, u"서식 출력")
        viewMenu.Append(ID_VIEW_FULL_SCREEN, u"전체화면\tF11")

        scriptMenu = wx.Menu()
        scriptMenu.Append(ID_SCRIPT_FIND_ERROR, u"다음 오류 탐색")
        scriptMenu.Append(ID_SCRIPT_PAGINATE, u"쪽 매기기")
        scriptMenu.AppendSeparator()
        scriptMenu.Append(ID_SCRIPT_AUTO_COMPLETION, u"자동 완성")
        scriptMenu.Append(ID_SCRIPT_HEADERS, u"머릿말")
        scriptMenu.Append(ID_SCRIPT_LOCATIONS, u"위치")
        scriptMenu.Append(ID_SCRIPT_TITLES, u"제목 페이지")
        scriptMenu.Append(ID_SCRIPT_SC_DICT, u"맞춤법 검사 사전")
        scriptMenu.AppendSeparator()

        tmp = wx.Menu()

        tmp.Append(ID_SCRIPT_SETTINGS_CHANGE, u"변경")
        tmp.AppendSeparator()
        tmp.Append(ID_SCRIPT_SETTINGS_LOAD, u"&설정 불러오기")
        tmp.Append(ID_SCRIPT_SETTINGS_SAVE_AS, u"&설정 내보내기")
        scriptMenu.AppendMenu(ID_SCRIPT_SETTINGS, u"설정", tmp)
        scriptSettingsMenu = tmp

        reportsMenu = wx.Menu()
        reportsMenu.Append(ID_REPORTS_SCRIPT_REP, u"스크립트 보고서")
        reportsMenu.Append(ID_REPORTS_LOCATION_REP, u"위치 보고서")
        reportsMenu.Append(ID_REPORTS_SCENE_REP, u"씬 보고서")
        reportsMenu.Append(ID_REPORTS_CHARACTER_REP, u"인물 보고서")
        reportsMenu.Append(ID_REPORTS_DIALOGUE_CHART, u"다이얼로그 차트")

        toolsMenu = wx.Menu()
        toolsMenu.Append(ID_TOOLS_SPELL_CHECK, u"맞춤법 검사")
        toolsMenu.Append(ID_TOOLS_NAME_DB, u"이름 정보")
        #toolsMenu.Append(ID_TOOLS_CHARMAP, "&Character map...")
        toolsMenu.Append(ID_TOOLS_COMPARE_SCRIPTS, u"스크립트 비교")
        toolsMenu.Append(ID_TOOLS_WATERMARK, u"워터마크 관리")

        helpMenu = wx.Menu()
        helpMenu.Append(ID_HELP_COMMANDS, u"명령어")
        helpMenu.Append(ID_HELP_MANUAL, u"사용 설명서")
        helpMenu.AppendSeparator()
        helpMenu.Append(ID_HELP_ABOUT, u"정보")

        self.menuBar = wx.MenuBar()
        self.menuBar.Append(fileMenu, u"파일")
        self.menuBar.Append(editMenu, u"편집")
        self.menuBar.Append(viewMenu, u"보기")
        self.menuBar.Append(scriptMenu, u"스크립트")
        self.menuBar.Append(reportsMenu, u"보고서")
        self.menuBar.Append(toolsMenu, u"도구")
        self.menuBar.Append(helpMenu, u"도움말")
